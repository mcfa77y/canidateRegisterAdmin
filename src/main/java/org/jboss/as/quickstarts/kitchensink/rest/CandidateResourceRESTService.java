 ...
 * JAX-RS Example
 * 
 * This class produces a RESTful service to read the contents of the candidates
 * table.
 */
@Path("/candidates")
@RequestScoped
public class CandidateResourceRESTService {
	@Inject
	...
	public List<Candidate> listAllCandidates() {
		...
		final List<Candidate> results = em.createQuery(
				"select m from Candidate m order by m.name").getResultList();
		return results;
	}
